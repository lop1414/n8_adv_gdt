...
class VideoController extends GdtController
{
    /**
     * constructor.
     */
    ...
    public function upload(Request $request){
        ...
        $curlFile = curl_file_create($file->getRealPath());
    ...
    public function batchUpload(Request $request){
        ...
        $taskGdtVideoUploadService = new TaskGdtVideoUploadService();
        $has = $taskGdtVideoUploadService->hasAdminUserWaitingTask($adminUserInfo['admin_user']['id']);
        if($has){
            throw new CustomException([
...
//        }
        ...
        $builder = $gdtAccountModel->withPermission()->whereIn('account_id', $accountIds);
        ...
        $waitingTaskCount = $taskGdtVideoUploadService->getWaitingTaskCount();
        ...
        $taskGdtVideoUploadService->create($task, $subs);

        return $this->success([
            'task_id' => $taskGdtVideoUploadService->taskId,
            'account_count' => $accounts->count(),
            'video_count' => count($videos),
        ], [], '批量上传任务已提交【任务id:'. $taskGdtVideoUploadService->taskId .'】，前方还有'. $waitingTaskCount .'个任务待执行，执行结果后续同步到飞书，请注意查收！');
    }
}
